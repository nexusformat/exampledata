...
def make_class_as_nf_file(class_nm, dest_dir, symbol_dct={}):
    ...
    fpath = os.path.join(dest_dir, '%s.hdf5' % class_nm)
...
def write_script_files(class_nm):
    mod_name = 'h5py'
    write_h5py_script(mod_name, os.path.join(os.getcwd(), '..', 'autogenerated_examples','nxdl', 'python_scripts', mod_name), class_nm, h5py_script_lst)
    mod_name = 'nexusformat'
    write_nxsfrmt_script(os.path.join(mod_name, os.getcwd(), '..', 'autogenerated_examples','nxdl', 'python_scripts', mod_name), class_nm, nxsfrmt_script_lst)
...
def write_script(mod_name, path, class_nm, script_lst):
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    f = open(os.path.join(path, 'ex_%s_%s.py' % (mod_name,class_nm)), 'w')
    for l in script_lst:
        f.write(l + '\n')
    f.close()
...
def build_class_db(class_dir='base_classes', desired_class=None, defdir=None, sym_args_dct={},report_symbols_only=False):
    ...
    for nxdl_file in nxdl_files:
        ...
        print('\nProcessing [%s]' % nxdl_file.replace(os.path.dirname(__file__),'.'))
