...
The file generated by this script is intended to give the user an example of a file that conforms
 to the [%s] application definition such that it will pass validation with cnxvalidate. There are some attributes that 
 have been added to help the user know what is a required field or group, for example if a group or field is required 
 there will exist an attribute called required that will be set to True, if it is not required it will be set to False. 
 Where the definition specifies a doc string for a field or group that doc string will also be included as a string 
 attribute for that field or group. 
 
 Fields or groups in the generated file that have 'untitled_' as a prefix are intended to indicate to the 
 user that in the file they produce they need to replace the name of that field with the name they want.  
 
 There is a 1 to 1 relationship between the generated file and the release version of the NEXUS application definitions 
 that the user has installed, each generated file contains an attribute called disclaimer that says what release version 
 of the application definitions was used to generate the file. If the user updates their application definitions then 
 they should also run nxdl_to_hdf5.py to regenerate the files. 
 
 The auto generated files are placed in <nexus definition repo>/exampledata/autogenerated_examples/nxdl/applications.
...
def make_class_as_nf_file(clss_nm, path_dct, dest_dir, docs, symbol_dct = {}):
    ...
    if(res):
        ...
        _dataset(nf, 'README', readme_string % (rel_ver, clss_nm), 'NX_CHAR', nx_units='NX_ANY', dset={})
        nf.close()
